...
async function bootstrap() {
  try {
    ...
    console.log(colors.bgCyan + colors.bright + colors.white + '  🚀  SHIPPING SERVICE STARTED SUCCESSFULLY  🚀  ' + colors.reset);
    ...
    const dbHost = process.env.SHIPPING_DB_HOST || 'localhost';
    const dbPort = process.env.SHIPPING_DB_PORT || (dbHost === 'localhost' ? '5441' : '5432');
    const dbName = process.env.SHIPPING_DB_NAME || 'shipping_db';
    ...
    console.log(colors.dim + `  Shipping Service v1.0.0 | Port: ${port} | Environment: ${process.env.NODE_ENV || 'development'}` + colors.reset);
    console.log(colors.dim + '───────────────────────────────────────────────────────────────' + colors.reset + '\n');
  } catch (error) {
    console.error(colors.red + colors.bright + '❌ Failed to start Shipping Service:' + colors.reset, error);
    process.exit(1);
  }
}
