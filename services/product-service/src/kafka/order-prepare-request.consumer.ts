...
const ORDER_PREPARE_REQUEST_TOPIC = 'order.prepare.request';
const ORDER_PREPARE_REPLY_TOPIC = 'order.prepare.reply';
const RESPONDER_ID = 'product-service';
...
export class OrderPrepareRequestConsumer implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    const kafka = new Kafka({
      clientId: 'product-service-order-prepare',
      brokers: (process.env.KAFKA_BROKERS || 'localhost:9092').split(','),
    });
    this.consumer = kafka.consumer({ groupId: 'product-service-order-prepare-group' });
